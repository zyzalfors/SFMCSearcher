...
export class Script {
  ...
  static async Load(stack, BUid, BUname) {
    const folders = await Utility.Utility.GetFolders(stack, ["ssjsactivity"]);
    const pageSize = 500;
    let page = 1, pageItems = [0];
    while(pageItems.length > 0) {
      const data = [];
      const pageData = await Utility.Utility.FetchJSON("https://mc.s" + stack + ".exacttarget.com/cloud/fuelapi/automation/v1/scripts/?$page=" + page + "&$pagesize=" + pageSize);
